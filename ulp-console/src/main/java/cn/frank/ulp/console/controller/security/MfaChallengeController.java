 ...
 * 前者是常态，后者是给前端在 cookie 被禁/丢失时的回退路径。
 *
 * <p>响应 / HTTP 语义（与 spec.md Phase 6 audit failure_reason 对齐）：
 * <ul>
 *   <li>200 {@code status=ok} — 验证通过，Authentication 已写入 session；前端跳 portal/console 首页</li>
 *   <li>401 {@code status=invalid_otp} — code 不匹配，cookie 保留，前端提示重输</li>
 ...
 *   <li>401 {@code status=challenge_session_invalid} — IP 不在 /24 同段 / 主体类型无路由 / 已失绑，cookie 清除，必须重新主认证</li>
 *   <li>423 {@code status=locked_out} + {@code Retry-After} 头 — 失败计数达到阈值（5 次/15 分钟），cookie 保留，等窗口期到</li>
 * </ul>
 *
 * <p>Phase 6.5 审计接线：每个 outcome 分支发对应 {@link PortalEventType} 事件，失败路径
 * 通过 {@link MfaChallengeService#peekPendingAuthentication(String)} 取回 parked
 * {@link Authentication} 显式构造 {@link Actor}，避免 SecurityContext 仍空时
 * {@link AuditEventPublish#getActor()} NPE。anonymous fallback 在 console 端使用
 * {@link UserType#ADMIN}，与 portal 端 {@link UserType#USER} 区分。
 *
 * <p>本端点不要求 {@code authenticated} —— 处于"已主认证但未二次认证"的中间态，由
 * {@code ConsoleSecurityConfiguration} 在 chain 中开放放行。安全实质性靠 cookie 中的
 * 一次性 UUID + 5 分钟 TTL + Redis pending 记录的 /24 IP 绑定共同兜底。
 */
@RestController
...
public class MfaChallengeController {
    ...
    public ResponseEntity<ApiRestResult<Map<String, Object>>> challenge(@RequestBody(required = false) ChallengeRequest body,
        ...
        if (StringUtils.isBlank(challengeId) || StringUtils.isBlank(payload.code())) {
            ...
            mfaMetrics.verifyOutcome(SUBJECT_ADMIN, "totp", MfaChallengeOutcome.CHALLENGE_EXPIRED);
        ...
        MfaChallengeOutcome outcome = challengeService.verifyAndCommit(challengeId, payload.code(),
            request, response);
        mfaMetrics.verifyOutcome(SUBJECT_ADMIN, "totp", outcome);
        return switch (outcome) {
            ...
            case LOCKED_OUT -> {
                ...
                mfaMetrics.lockout(SUBJECT_ADMIN, "challenge");
    ...
    private ResponseEntity<ApiRestResult<Map<String, Object>>> handleBackupCode(String challengeId,
        ...
        if (StringUtils.isBlank(challengeId)) {
            publishVerifyFailure(challengeId, MfaChallengeOutcome.CHALLENGE_EXPIRED);
            mfaMetrics.verifyOutcome(SUBJECT_ADMIN, "backup",
                MfaChallengeOutcome.CHALLENGE_EXPIRED);
            return errorResponse(HttpStatus.UNAUTHORIZED, MfaChallengeOutcome.CHALLENGE_EXPIRED,
                true, request, response);
        }
        BackupCodeChallengeResult result = challengeService.verifyBackupCodeAndCommit(challengeId,
            backupCode, request, response);
        mfaMetrics.verifyOutcome(SUBJECT_ADMIN, "backup", result.outcome());
    ...
    private Actor buildActor(Authentication auth) {
        ...
        return Actor.builder().id("anonymous").type(UserType.ADMIN).build();
    }
