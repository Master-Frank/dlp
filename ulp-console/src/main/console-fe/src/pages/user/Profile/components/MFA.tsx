...
const MfaView = () => {
  ...
  const onUnbindOk = async () => {
    try {
      ...
      if (success) {
        message.success(intl.formatMessage({ id: 'page.user.profile.mfa.unbind.success' }));
        setUnbindOpen(false);
        form.resetFields();
        setRefresh(true);
        return;
      ...
      if (status === 'unbind_blocked_by_org_policy') {
        message.error(intl.formatMessage({ id: 'page.user.profile.mfa.unbind.blocked' }));
        return;
      }
  ...
  const data = [
    {
      title: intl.formatMessage({ id: 'page.user.profile.menu.mfa' }),
      description: bound
        ? intl.formatMessage({ id: 'page.user.profile.mfa.status.enabled' })
        : intl.formatMessage({ id: 'page.user.profile.mfa.status.disabled' }),
      tag: bound ? (
        <Tag color="success">{intl.formatMessage({ id: 'page.user.profile.mfa.status.enabled' })}</Tag>
      ) : (
        <Tag>{intl.formatMessage({ id: 'page.user.profile.mfa.status.disabled' })}</Tag>
      ),
      actions: [
        bound ? (
          <a key="unbind" onClick={() => setUnbindOpen(true)}>
            {intl.formatMessage({ id: 'page.user.profile.mfa.unbind' })}
          </a>
        ) : (
          <a key="bind" onClick={onBindClick}>
            {intl.formatMessage({ id: 'page.user.profile.mfa.bind' })}
          </a>
        ),
      ],
    },
  ...
  return (
    <Skeleton loading={loading} paragraph={{ rows: 4 }}>
      <Alert
        type="info"
        showIcon
        message={intl.formatMessage({ id: 'page.user.profile.mfa.desc' })}
      ...
      />
      <Modal
        title={intl.formatMessage({ id: 'page.user.profile.mfa.unbind' })}
        open={unbindOpen}
      ...
      >
        <Alert
          type="warning"
          showIcon
          message={intl.formatMessage({ id: 'page.user.profile.mfa.unbind.confirm' })}
          style={{ marginBottom: 16 }}
        />
        <Form form={form} layout="vertical">
          <Form.Item
            name="otp"
            label={intl.formatMessage({ id: 'page.user.profile.mfa.unbind.otp.placeholder' })}
            rules={[
              {
                required: true,
