...
export default {
  ...
  'pages.mfa.setup.scan.tip': '请用 Google Authenticator / 1Password / Microsoft Authenticator 等 TOTP 工具扫描下方二维码，或手动输入密钥。',
  ...
  'pages.mfa.backup.warning.desc': '丢失认证器时，可使用任意一个备份码完成登录。备份码不会再次显示，离开此页前务必下载或抄录。',
  ...
  'page.account.menu.mfa': '二次验证 (MFA)',
  'pages.account.menu.mfa': '二次验证 (MFA)',
  'pages.account.mfa.status.enabled': '已启用',
  'pages.account.mfa.status.disabled': '未启用',
  'pages.account.mfa.bind': '绑定 MFA',
  'pages.account.mfa.unbind': '解绑 MFA',
  'pages.account.mfa.unbind.confirm': '解绑后下次登录将不再要求 MFA。确定继续？',
  'pages.account.mfa.unbind.otp.placeholder': '请输入当前动态码以确认',
  'pages.account.mfa.unbind.blocked_by_org': '所在组织已启用强制 MFA，无法解绑',
  'pages.account.mfa.unbind.success': '已解绑 MFA',
  'pages.account.mfa.desc': '为账户启用基于时间的一次性密码（TOTP），可有效阻止账号被盗用后的非授权登录。',
};
