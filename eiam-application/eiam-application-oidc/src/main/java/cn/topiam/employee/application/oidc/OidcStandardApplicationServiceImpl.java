...
public class OidcStandardApplicationServiceImpl extends AbstractOidcApplicationService {
    ...
    public String create(String name, String icon, String remark, List<String> groups) {
        ...
        entity.setResponseTypes(Sets.newHashSet("code", "token", "id_token", "token id_token"));
        //重定向URLs
        ...
        //授权码 生存时间 默认（5分钟）
        entity.setAuthorizationCodeTimeToLive(Duration.ofMinutes(5));
        //设备授权码 生存时间（默认）5分钟
        entity.setDeviceCodeTimeToLive(Duration.ofMinutes(5));
        //刷新 Token 过期时间 默认（30天）
        entity.setRefreshTokenTimeToLive(Duration.ofMinutes(43200));
        //刷新 Token 过期时间 默认（30天）
        entity.setRefreshTokenTimeToLive(Duration.ofMinutes(43200));
        //访问 Token 过期时间 默认（20分钟）
        entity.setAccessTokenTimeToLive(Duration.ofMinutes(20));
        //Access Token 格式
        entity.setAccessTokenFormat("reference");
        //30分钟
        entity.setIdTokenTimeToLive(Duration.ofMinutes(30));
        //ID Token签名算法
    ...
    public void saveConfig(String appId, Map<String, Object> config) {
        ...
        //2、修改 OIDC 配置
        Optional<AppOidcConfigEntity> oidc = appOidcConfigRepository.findByAppId(appId);
        if (oidc.isEmpty()) {
            AuditContext.setContent("保存配置失败，应用 [" + appId + "] 不存在！");
            logger.error(AuditContext.getContent());
            throw new AppNotExistException();
    ...
    public Object getConfig(String appId) {
        AppOidcConfigPO po = appOidcConfigRepository.getByAppId(appId);
    ...
    protected OidcStandardApplicationServiceImpl(AppCertRepository appCertRepository,
        ...
        super(appCertRepository, appGroupAssociationRepository, appAccountRepository,
            appAccessPolicyRepository, appRepository, appOidcConfigRepository);
